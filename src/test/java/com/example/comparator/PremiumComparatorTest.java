...
class PremiumComparatorTest {
    @Test
    void premiumComparator_Ok() {
        Queue<User> users = new PriorityQueue<>(new PremiumComparator());
        ...
        List<User> expected = List.of(user1, user4, user3, user5, user2);
        users.add(user1);
        users.add(user2);
        users.add(user3);
        users.add(user4);
